...
export default class WebtoonStore {
      ...
      if (this.option.saveWebtoonSort) {
        if (JSON.stringify(this._sortWebtoon) === JSON.stringify({})) {
          ...
          Object.keys(webtoons).forEach((key: Week) => {
            ...
            // 정렬
            webtoons[key].sort((a, b) =>
              sorted.indexOf(a.id) > sorted.indexOf(b.id) ? 1 : -1
            );
   ...
   * @readonly
   */
  @computed get allWebtoon(): WebtoonInfoType[] {
    const webtoons: { [key: number]: WebtoonInfoType } = {};
  ...
  @observable
  public completeWebtoons: WebtoonInfoType[] = [];

  @action
  private setCompeteWebtoon() {
    WebtoonRequest.getCompleteWebtoon().then(value => {
  ...
  @action setVisitsFromChrome() {
    ...
    whale.history.search(
      ...
      data => {
        ...
        if (window["UIkit"]) {
          UIkit.notification(
            `<div class="uk-text-small">방문기록에서 기록을 불러왔습니다.</div>`,
            {
              timeout: 2000
            }
          );
